    ...
    <div className="space-y-2">
            ...
            <React.Fragment key={step.key}>
              <div className="flex items-center gap-1.5">
                ...
                <span
                  className={cn(
                    "text-[11px]",
                    (isDone || isActive) ? "font-medium" : "opacity-40"
                  )}
      ...
      <div className="h-1.5 rounded-full overflow-hidden" style={{ backgroundColor: "var(--border)" }}>
        <div
          ...
          style={{
            width: `${progress}%`,
            backgroundColor: status === "done" ? "#22c55e" : status === "error" ? "#ef4444" : "#3b82f6",
          }}
        />
      </div>
    </div>
  ...
  return (
    <div
      className={cn(
        ...
        dragOver ? "border-blue-500 bg-blue-500/10" : "hover:border-blue-500/50"
      )}
    ...
    >
      ...
      <p className="text-xs mt-1 opacity-50" style={{ color: "var(--foreground)" }}>
        Supports PDF, DOCX, XLSX, PPTX, PNG, JPG, CSV, HTML, TXT
      </p>
    </div>
  );
}
  ...
  return (
    ...
    >
      <div
        ...
        style={{ backgroundColor: getFormatColor(file.format) }}
      >
        {file.format}
      </div>
      <div className="flex-1 min-w-0">
        ...
        {showProgress && file.status !== "queued" && (
          <div className="mt-1.5">
            <ConversionProgress status={file.status} progress={file.progress} />
          </div>
        )}
        {file.result && (
          <div className="flex items-center gap-2 mt-1.5">
            ...
            <span className="text-xs text-green-500 font-medium">
              {file.result.name} ({formatBytes(file.result.size)})
            </span>
...
function ConversionGridTab() {
  const {
    ...
    removeFile, clearFiles, startConversion,
  } = useConverterStore();
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center gap-3">
          <button
            onClick={() => { setSelectedConversion(null); clearFiles(); }}
            className="text-sm px-3 py-1.5 rounded-lg hover:bg-white/10 transition-colors"
            style={{ color: "var(--foreground)", backgroundColor: "var(--sidebar)" }}
          >
            Back
          </button>
          <div className="flex items-center gap-2">
            <div
              ...
              style={{ backgroundColor: getFormatColor(selectedConversion.from) }}
            >
            ...
            <div
              className="px-2.5 py-1 rounded text-xs font-bold text-white uppercase"
              style={{ backgroundColor: getFormatColor(selectedConversion.to) }}
            >
        ...
        {files.length > 0 && (
          <div className="space-y-2">
            ...
            {files.map((f) => (
              <FileRow key={f.id} file={f} onRemove={() => removeFile(f.id)} showProgress />
            ))}
        ...
        {categories.map((cat) => (
          <button
            ...
            className={cn(
              ...
              selectedCategory === cat ? "text-white" : "hover:bg-white/5"
            )}
            style={
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
        {filtered.map((card) => {
          ...
          return (
            <button
              key={card.id}
              onClick={() => setSelectedConversion(card)}
              className="p-4 rounded-xl border text-left transition-all hover:scale-[1.02] hover:shadow-lg group"
              style={{
            ...
            >
              <div className="flex items-center gap-2 mb-3">
                <div
                  ...
                  style={{ backgroundColor: `${getFormatColor(card.from)}20` }}
                >
              ...
              <div className="flex items-center gap-1.5 mb-1">
                <span
                  ...
                  style={{ backgroundColor: getFormatColor(card.from) }}
                >
                  {card.from}
                </span>
                ...
                <span
                  className="text-[10px] font-bold uppercase px-1.5 py-0.5 rounded text-white"
                  style={{ backgroundColor: getFormatColor(card.to) }}
                >
                  {card.to}
                </span>
              </div>
...
function PdfToolsTab() {
  const {
    pdfOperation, setPdfOperation, addPdfFiles, removePdfFile,
    clearPdfFiles, setPdfOptions, startPdfOperation,
  } = useConverterStore();

  const handleFiles = useCallback(
    (dropped: File[]) => {
      const convFiles = dropped.map((f) => makeConversionFile(f, null));
      addPdfFiles(convFiles);
    },
    [addPdfFiles]
  );
      ...
      <div className="space-y-4">
          ...
          <div className="p-3 rounded-lg space-y-2" style={{ backgroundColor: "var(--sidebar)" }}>
            ...
            <div className="flex gap-2">
              {["low", "medium", "high"].map((q) => (
                <button
                  key={q}
                  onClick={() => setPdfOptions({ quality: q })}
                  className={cn(
                    "px-3 py-1.5 rounded text-xs font-medium capitalize transition-colors",
                    pdfOperation.options.quality === q ? "text-white" : "hover:bg-white/5"
                  )}
                  style={
                    pdfOperation.options.quality === q
                      ? { backgroundColor: "var(--primary)" }
          ...
          <div className="p-3 rounded-lg space-y-2" style={{ backgroundColor: "var(--sidebar)" }}>
            ...
            <div className="flex gap-2">
              {[90, 180, 270].map((deg) => (
                <button
                  key={deg}
                  onClick={() => setPdfOptions({ degrees: deg })}
                  className={cn(
                    "px-3 py-1.5 rounded text-xs font-medium transition-colors",
                    pdfOperation.options.degrees === deg ? "text-white" : "hover:bg-white/5"
                  )}
                  style={
                    pdfOperation.options.degrees === deg
                      ? { backgroundColor: "var(--primary)" }
                      : { color: "var(--foreground)" }
                  }
                >
        ...
        {pdfOperation.operation === "watermark" && (
          <div className="p-3 rounded-lg space-y-2" style={{ backgroundColor: "var(--sidebar)" }}>
            ...
            <input
              ...
              value={(pdfOperation.options.text as string) || ""}
              onChange={(e) => setPdfOptions({ text: e.target.value })}
        ...
        {(pdfOperation.operation === "encrypt" || pdfOperation.operation === "decrypt") && (
          <div className="p-3 rounded-lg space-y-2" style={{ backgroundColor: "var(--sidebar)" }}>
            ...
            <input
              ...
              value={(pdfOperation.options.password as string) || ""}
              onChange={(e) => setPdfOptions({ password: e.target.value })}
              className="w-full px-3 py-2 rounded text-sm bg-transparent border outline-none"
              style={{ color: "var(--foreground)", borderColor: "var(--border)" }}
            />
          </div>
          ...
          <div className="p-3 rounded-lg space-y-2" style={{ backgroundColor: "var(--sidebar)" }}>
            ...
            <div className="flex gap-2">
              {["every-page", "by-range", "by-size"].map((mode) => (
                <button
                  key={mode}
                  onClick={() => setPdfOptions({ splitMode: mode })}
                  className={cn(
                    "px-3 py-1.5 rounded text-xs font-medium capitalize transition-colors",
                    pdfOperation.options.splitMode === mode ? "text-white" : "hover:bg-white/5"
                  )}
                  style={
                    pdfOperation.options.splitMode === mode
                      ? { backgroundColor: "var(--primary)" }
                      : { color: "var(--foreground)" }
                  }
                >
        ...
        {pdfOperation.files.length > 0 && (
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-xs font-medium opacity-60" style={{ color: "var(--foreground)" }}>
                {pdfOperation.files.length} file{pdfOperation.files.length > 1 ? "s" : ""}
              </span>
              <button onClick={clearPdfFiles} className="text-xs text-red-400 hover:text-red-300">
                Clear
              </button>
            </div>
            {pdfOperation.files.map((f) => (
              <FileRow key={f.id} file={f} onRemove={() => removePdfFile(f.id)} />
            ))}

            {pdfOperation.status && (
              <div className="mt-3">
                <ConversionProgress status={pdfOperation.status} progress={pdfOperation.progress} />
              </div>
            )}

            {!pdfOperation.status && (
              <button
                onClick={startPdfOperation}
                className="w-full py-2.5 rounded-lg text-sm font-medium text-white transition-colors flex items-center justify-center gap-2"
                style={{ backgroundColor: "var(--primary)" }}
              >
                <Zap className="w-4 h-4" />
            ...
            {pdfOperation.status === "done" && (
              <button className="w-full py-2.5 rounded-lg text-sm font-medium text-white bg-green-600 hover:bg-green-500 transition-colors flex items-center justify-center gap-2">
  ...
  return (
    <div className="space-y-4">
      <p className="text-xs opacity-60" style={{ color: "var(--foreground)" }}>
        Advanced PDF tools for merging, splitting, compressing, and more.
      </p>
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
        {pdfOperations.map((op) => {
          ...
          return (
            <button
              key={op.id}
              onClick={() => setPdfOperation(op.id)}
              className="p-4 rounded-xl border text-left transition-all hover:scale-[1.02] hover:shadow-lg"
            ...
            >
              ...
              <p className="text-[11px] opacity-50 mt-0.5" style={{ color: "var(--foreground)" }}>
                {op.description}
              </p>
            </button>
          );
...
function BatchConvertTab() {
  const {
    batchFiles, batchTargetFormat, setBatchTargetFormat,
    addBatchFiles, removeBatchFile, clearBatchFiles, startBatchConversion,
  } = useConverterStore();
  ...
  const handleFiles = useCallback(
    (dropped: File[]) => {
      const convFiles = dropped.map((f) => makeConversionFile(f, batchTargetFormat));
      addBatchFiles(convFiles);
    },
    [batchTargetFormat, addBatchFiles]
  );

  return (
    <div className="space-y-4">
      <p className="text-xs opacity-60" style={{ color: "var(--foreground)" }}>
        Convert multiple files at once to a single output format.
      </p>
        ...
        <div className="flex gap-1.5 flex-wrap">
          {targetFormats.map((fmt) => (
            <button
              ...
              className={cn(
                "px-2.5 py-1.5 rounded text-xs font-bold uppercase transition-colors",
                batchTargetFormat === fmt ? "text-white" : "hover:bg-white/5"
              )}
              style={
                batchTargetFormat === fmt
                  ? { backgroundColor: getFormatColor(fmt) }
      ...
      {batchFiles.length > 0 && (
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium opacity-60" style={{ color: "var(--foreground)" }}>
              {batchFiles.length} file{batchFiles.length > 1 ? "s" : ""} in batch
            </span>
            <button onClick={clearBatchFiles} className="text-xs text-red-400 hover:text-red-300">
              Clear All
            </button>
          </div>
          {batchFiles.map((f) => (
            <FileRow key={f.id} file={f} onRemove={() => removeBatchFile(f.id)} showProgress />
          ))}
          {batchTargetFormat && batchFiles.some((f) => f.status === "queued") && (
            ...
            >
              ...
              Convert All to {batchTargetFormat.toUpperCase()}
            </button>
          )}
        </div>
      )}
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        {recentConversions.length > 0 && (
          <button onClick={clearRecentConversions} className="text-xs text-red-400 hover:text-red-300">
            Clear History
          </button>
        )}
      </div>
          ...
          <table className="w-full">
            <thead>
              <tr style={{ backgroundColor: "var(--sidebar)" }}>
                <th
                  className="text-left text-[11px] font-medium uppercase tracking-wider px-4 py-2.5"
              ...
              {recentConversions.map((r, i) => (
                ...
                >
                  <td className="px-4 py-3">
                    <p className="text-sm font-medium truncate max-w-[200px]" style={{ color: "var(--foreground)" }}>
                      {r.fileName}
                    </p>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1.5">
                      <span
                        className="text-[10px] font-bold uppercase px-1.5 py-0.5 rounded text-white"
                        style={{ backgroundColor: getFormatColor(r.fromFormat) }}
                      >
                        {r.fromFormat}
                      </span>
                      <ArrowRight className="w-3 h-3 opacity-40" style={{ color: "var(--foreground)" }} />
                      <span
                        className="text-[10px] font-bold uppercase px-1.5 py-0.5 rounded text-white"
                        style={{ backgroundColor: getFormatColor(r.toFormat) }}
                      >
                        {r.toFormat}
                      </span>
                    </div>
                  ...
                  <td className="px-4 py-3">
                    <span className="text-xs opacity-60" style={{ color: "var(--foreground)" }}>
                      {formatBytes(r.fileSize)} → {formatBytes(r.resultSize)}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <span className="text-xs opacity-60" style={{ color: "var(--foreground)" }}>
